...
export function About() {
  return (
    <section id="about" className="relative border-b border-divider bg-noir py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid items-start gap-10 lg:grid-cols-12">
          ...
          <Reveal className="lg:col-span-7" delay={0.05}>
            ...
            {/* Stats row */}
            <div className="mt-9 grid grid-cols-3 gap-4 border-t border-divider pt-7">
              {STATS.map((s) => (
                <div key={s.label}>
                  <div className="font-display text-3xl font-extrabold text-gold sm:text-4xl">
                    {s.num}
                  </div>
                  <div className="mt-1 text-xs text-khaki sm:text-sm">{s.label}</div>
                </div>
              ))}
            </div>
          </Reveal>
                ...
                <div className="space-y-0">
                  {CLAPPER_FIELDS.map((f, i) => (
                    ...
                    >
                      ...
                      <span className="text-start text-sm text-ivory sm:text-[15px]">
                        {f.value}
                      </span>
                    </div>
                  ))}
